...
def parse_value_with_tolerance(value, annotation):
    ...
    try:
        ...
        tracer.info(f"TypeAdapter validation failed (recoverable): {e}", contains_content=True)
            ...
            if item_type is not None:
                for item in parsed_value:
                    try:
                        ...
                        tracer.info(f"Skipping invalid list item: {item}", contains_content=True)
                        continue
...
def parse_json_with_stability(
    ...
    try:
        ...
        tracer.error(f"Layer 1 cleanup failed: {e}", contains_content=True)
    ...
    try:
        ...
        tracer.error(f"Layer 2 json_repair failed: {e}", contains_content=True)
    ...
    try:
        ...
        tracer.info(
            f"Direct model validation failed, trying parse_value_with_tolerance: {e}",
            contains_content=True,
        )
        tracer.info(f"content={content}", contains_content=True)
            ...
            for field_name, field_value in parsed_data.items():
                if field_name in field_types:
                    try:
                        ...
                        tracer.error(
                            f"Field {field_name} parsing failed: {field_e}",
                            contains_content=True,
                        )

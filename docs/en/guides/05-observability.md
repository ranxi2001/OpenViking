...
Routine diagnostic messages remain available as bounded `openviking.log` event
attributes. Prompts, model output, tool arguments, memory bodies, and other payload
content are omitted by default. Set `capture_content` to `true` only when that content
is required for troubleshooting. Exported text is subject to best-effort credential
and inline-image redaction, then limited to `content_max_length` characters (1-65536).
It can still contain user data or other sensitive values, so restrict access to trace
exports and remove them when the investigation is complete.
